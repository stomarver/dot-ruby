...
public class Chk {
    ...
    private static final int LAYER_SIZE = SIZE * SIZE;

    private final byte[] blocks;
    public final int x, z;

    public Chk(int x, int z) {
        this.x = x;
        this.z = z;
    ...
    public void setBlock(int lx, int ly, int lz, byte type) {
        if (lx >= 0 && lx < SIZE && ly >= 0 && ly < HEIGHT && lz >= 0 && lz < SIZE) {
            blocks[toIndex(lx, ly, lz)] = type;
        }
    }

    public byte getBlock(int lx, int ly, int lz) {
        if (lx < 0 || lx >= SIZE || lz < 0 || lz >= SIZE || ly < 0 || ly >= HEIGHT) return 0;
        return blocks[toIndex(lx, ly, lz)];
    }

    private int toIndex(int lx, int ly, int lz) {
        return (ly * LAYER_SIZE) + (lz * SIZE) + lx;
    }
}
