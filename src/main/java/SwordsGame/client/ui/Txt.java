...
public class Txt {
    ...
    public Txt(Font f, int w, int h) {
        this.font = f;
        this.screenW = (float)w;
        this.screenH = (float)h;
    }

    public void draw(String t, Anc.TypeX ax, Anc.TypeX ay, float x, float y, float s) {
        draw(t, ax, toTypeY(ay), x, y, s);
    }

    public void draw(String t, Anc.TypeX ax, Anc.TypeX ay, float x, float y, float s, Wave wv) {
        draw(t, ax, toTypeY(ay), x, y, s, wv);
    }

    public void draw(String t, Anc.TypeX ax, Anc.TypeY ay, float x, float y, float s) {
        drawInternal(t, buildAnchor(ax, ay), x, y, s, true, Shake.NONE, Wave.NONE, Crit.NONE, 0);
    }

    public void draw(String t, Anc.TypeX ax, Anc.TypeY ay, float x, float y, float s, Wave wv) {
        drawInternal(t, buildAnchor(ax, ay), x, y, s, true, Shake.NONE, wv, Crit.NONE, 0);
    }

    public void draw(String t, Anc a, float x, float y, float s) {
    ...
    private void drawInternal(String txt, Anc a, float ox, float oy, float scale,
        ...
        float sy = a.y + oy;
        if (a.ty == Anc.TypeY.CENTER) sy -= totalH / 2f;
        else if (a.ty == Anc.TypeY.BOTTOM) sy -= totalH;
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (a.tx == Anc.TypeX.CENTER) rx -= lw / 2f;
            else if (a.tx == Anc.TypeX.RIGHT) rx -= lw;
