...
public class Win {
    ...
    public void create() {
        ...
        System.out.println("[Vid] Win created");
    }
    ...
    public void update() {
        if (screenshotRequested) {
            SwordsGame.client.utils.Shot.takeScreenshotFromBackBuffer(framebufferWidth, framebufferHeight);
    ...
    private void setWindowIcon(String resourcePath) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (iconBuffer == null) {
                System.err.println("[Vid] Win icon decode failed: " + stbi_failure_reason());
                return;
            }
            ...
            System.out.println("[Vid] Win icon set: /" + resourcePath);
        } catch (Exception e) {
            System.err.println("[Vid] Win icon skipped: " + e.getMessage());
        } finally {
            if (iconBuffer != null) {
    ...
    private byte[] readResourceBytes(String path) throws Exception {
        ...
        for (String candidate : classpathCandidates) {
            try (InputStream input = Win.class.getResourceAsStream(candidate)) {
                if (input == null) {
                    continue;
                }
    ...
    public void destroy() {
        ...
        System.out.println("[Sys] Win destroyed");
    }
}
